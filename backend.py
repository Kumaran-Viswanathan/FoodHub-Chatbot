...
def wrapped_db_query(query_str):
    # The db_agent.invoke directly generates a dictionary with 'output' which is the natural language response
    # The SQL query itself is generated internally by the LLM within the db_agent.
    # To apply is_query_safe, we need to inspect the *thought process* of the SQL agent or the generated SQL.
    # However, create_sql_agent does not directly expose the generated SQL before execution in this simplified setup.
    # A more robust solution would involve a custom SQL agent that exposes the generated SQL for validation.
    # For this current setup, we will rely on the output guardrail to catch inappropriate responses that might result from aggregate queries.
    # If the LLM returns an aggregated answer, the output guardrail will need to be enhanced to catch it.

    # For a direct programmatic check on the *generated SQL*, a modification to how `create_sql_agent` works or a custom tool wrapping the SQL generation would be needed.
    # Given the current structure, we'll ensure `is_query_safe` is conceptually applied as an instruction, and output guardrail catches the final response.

    # Temporary workaround to illustrate where validation would ideally go:
    # In a real scenario, you'd intercept the SQL generated by db_agent.llm and validate it.
    # Since we can't directly intercept the SQL, we proceed and rely on output guardrail.

    response = db_agent.invoke({"input": query_str})
    return response['output'] # The output is directly the natural language response from the SQL agent
...
def apply_output_guardrail(response_text):
    ...
    for keyword in technical_leakage:
        if keyword in response_text:
            return "I'm sorry, I encountered a technical issue while formatting the response. Please try again or ask for a human agent."
    # Adding a check for aggregate-like responses
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        return ChatResponse(answer="I'm sorry, I encountered a technical glitch while processing your request. Please try again later or contact our support team.")
